"""
Privacy-preserving diagnostic for the back side's free-text OCR step
(profession, gender, religion, marital_status). Runs the exact same
clustered-region OCR pass the pipeline uses on your real photo, but only
prints structural info back to your terminal - detection position,
confidence score, and text LENGTH (not the actual recognized text) - so
this is safe to paste back without sharing your profession/religion/etc
content itself.
...
Usage:
    .audit_venv\\Scripts\\python.exe scripts\\debug_ocr_regions_back.py "path\\to\\back.jpg"
"""
import sys
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: debug_ocr_regions_back.py <path-to-back-photo>")
        sys.exit(1)
    ...
    free_text_boxes = {
        name: box for name, box in analyzer.BACK_FIELDS.items()
    ...
    print("Field boxes:", free_text_boxes, "\n")
